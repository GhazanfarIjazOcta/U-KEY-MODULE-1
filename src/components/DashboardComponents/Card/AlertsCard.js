  ...
  useEffect(() => {
    ...
    const unsubscribe = onValue(alertsRef, (snapshot) => {
      if (snapshot.exists()) {
        ...
        const pendingAlerts = formattedData.filter(
          (alert) => alert.status === "Pending"
        );
        const completedAlerts = formattedData.filter(
  ...
  const renderAlert = (alert, statusColor) => (
    ...
    >
      <Box sx={{ display: "flex", gap: "1rem" }}>
        ...
        <Typography
          sx={{ fontFamily: "Poppins", fontSize: "0.75rem", color: "#373737" }}
        >
    ...
    >
      ...
      <Box sx={{ display: "flex", gap: "1.5rem", alignItems: "center" }}>
        ...
        >
          ...
          >
            <span style={{ fontSize: "0.6rem", color: "#FFF" }}>
              {newAlerts.length}
            </span>
          </Box>
        </Box>
      ...
      >
        <Typography
          sx={{ fontFamily: "Poppins", fontSize: "0.6rem", color: "#7C7C7C" }}
        >
          New
        </Typography>
